...
export default function QueueRenderer({ visual, className = '' }: QueueRendererProps) {
    const { target, elements = [], pointers = [], activeIndices = [], type } = visual;

    return (
        <div className={`queue-visualizer flex flex-col items-center justify-center w-full h-full relative ${className}`}>
            ...
            <div 
                className="relative flex flex-row items-center gap-2 p-6 bg-slate-950/20 border-2"
                style={{ 
                    minWidth: '360px', 
                    minHeight: '130px',
                    borderColor: 'rgba(51, 65, 85, 0.4)',
                    borderRadius: '12px',
                    borderLeftColor: 'rgba(51, 65, 85, 0.15)',
                    borderRightColor: 'rgba(51, 65, 85, 0.15)',
                ...
                <AnimatePresence initial={false}>
                    {elements.length === 0 ? (
                        ...
                        elements.map((el, i) => {
                            ...
                            const isFront = i === 0;
                            const isRear = i === elements.length - 1;
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {/* Position Indicators */}
                                    {pointers.length === 0 && (
                                        <div className="absolute font-mono text-[8px] font-black text-orange-400 whitespace-nowrap tracking-wider uppercase top-[110%]">
                                            {isFront && '↑ FRONT'}
                                            {isRear && i !== 0 && '↑ REAR'}
                                        </div>
                                    )}
                                    ...
                                    {pointers.filter(p => p.index === i).map((p) => (
                                        <div 
                                            key={p.name} 
                                            className="absolute font-mono text-[8px] font-black whitespace-nowrap tracking-wider uppercase bottom-[110%]"
